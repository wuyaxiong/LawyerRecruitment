...
public class ShouCangMeiWenAdapter extends BaseAdapter {
    private Context context;
    private ShouCang.InforBean inforBeen;
    List<String> list;
    public ShouCangMeiWenAdapter(Context context,ShouCang.InforBean inforBeen){
        this.context=context;
        this.inforBeen=inforBeen;
    }
    @Override
    public int getCount() {
        return inforBeen.getGoodslist().size();
    }

    @Override
    public Object getItem(int position) {
        return inforBeen.getGoodslist().get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (inforBeen.getGoodslist().get(position).get(0)!=null){
            //把javabean中的图片地址转化成list集合
            list = new ArrayList<String>();
            list.clear();
            String tupian = inforBeen.getGoodslist().get(position).get(0).getImage();
            if (tupian!=null){
            ...
            ((ViewHolder) holder).tv_companyname.setText(inforBeen.getGoodslist().get(position).get(0).getComments()+"条评论");
            ((ViewHolder) holder).tv_title.setText(inforBeen.getGoodslist().get(position).get(0).getTitle());
            ((ViewHolder) holder).tv_shijian.setText(DateUtil.converTime(inforBeen.getGoodslist().get(position).get(0).getTime()));
        }
