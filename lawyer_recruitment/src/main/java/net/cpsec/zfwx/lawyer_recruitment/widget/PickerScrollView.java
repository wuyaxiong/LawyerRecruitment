...
public class PickerScrollView extends View {
    ...
    /**
     * 是否循环
     * 必须在setSelected(int selected)之前调用
     */
    public boolean isCirculation = true;
    ...
    public void setOnSelectListener(onSelectListener listener, int actionId) {
        mSelectListener = listener;
        this.actionId = actionId;
    }

    private void performSelect() {
        if (mSelectListener != null && actionId != -1)
            mSelectListener.onSelect(mDataList.get(mCurrentSelected), actionId);
    }
    ...
    private void drawData(Canvas canvas) {
        ...
        if (null != mDataList && mDataList.size() > 0)
            if (indexs <= mDataList.size() - 1 && indexs >= 0) {
    ...
    private void doMove(MotionEvent event) {
        if (isCirculation) {
            ...
            if ((event.getY() - mLastDownY) < 0) {
                ...
                // 向下滑动时
            } else if ((event.getY() - mLastDownY) > 0) {
                if (mCurrentSelected == 0) {
                    return;
                } else {
                    mMoveLen += (event.getY() - mLastDownY);
    ...
    public interface onSelectListener {
        void onSelect(Pickers pickers, int actionId);
    }
}
